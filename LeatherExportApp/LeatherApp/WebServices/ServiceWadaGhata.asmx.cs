...
{
    /// <summary>
    /// Summary description for ServiceWadaGhata
    /// </summary>
    ...
    public class ServiceWadaGhata : System.Web.Services.WebService
    {

        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void FilterGet(int iDisplayLength, int iDisplayStart, int iSortCol_0, string sSortDir_0, string sSearch, string sSearch_1, string sSearch_2, string sSearch_3, string sSearch_4, string sSearch_5, string sSearch_6, string sSearch_7, string sSearch_8, string min, string max)
        {
            int filteredCount = 0;

            List<WadaGhata> WadaGhataList = new List<WadaGhata>();

            DataTable dt = WadaGhata_DA.Filter_Get_WadaGhata(iDisplayLength, iDisplayStart, iSortCol_0, sSortDir_0, sSearch, sSearch_1, sSearch_2, sSearch_3, sSearch_4, sSearch_5, sSearch_6, sSearch_7, sSearch_8, min, max);

            foreach (DataRow row in dt.Rows)
            {
                filteredCount = int.Parse(row["TotalCount"].ToString());
                WadaGhataList.Add(new WadaGhata
                {
        ...
        private int GetTotalCount()
        {
            ...
            {
                SqlCommand cmd =
                    new SqlCommand("select Count(*) from Wada_Ghata where IsDeleted=0", con);
                con.Open();
        ...
        public WadaGhata Get(int Id)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                _WadaGhata.Date2 = Convert.ToDateTime(row["Date"].ToString()).ToShortDateString();
        ...
        {
            if (WadaGhata_DA.Delete_WadaGhata_By_ID(Id) > 0)
            {
                DataTable dt = WadaGhata_DA.Get_WadaGhata_By_ID(Id);
        ...
        protected void UpdateDeliveryForWadaGhata(int Lot1, int Pcs, float Sqft, int Lot2, int Pcs2, float Sqft2)
        {
            int BalancePcs = Pcs;
